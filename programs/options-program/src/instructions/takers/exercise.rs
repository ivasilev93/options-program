...
impl ExerciseOption<'_> {
    pub fn handle(ctx: Context<ExerciseOption>, market_ix: u16, option_id: u8) -> Result<()> {
        ...
        let profit_usd = match OptionType::try_from(option.option_type).unwrap() {
            ...
            OptionType::PUT => {
                max(option.strike_price - price.price as u64, 0).checked_mul(option.quantity).unwrap()
            }
        };
        ...
        if profit_usd > 0 {
            ...
            user_payout = min(profit_in_tokens, option.max_potential_payout_in_tokens);
            ...
            if user_payout <= market.premiums {
                market.premiums = market.premiums
                .checked_sub(user_payout)
                .ok_or(CustomError::Overflow)?;
            } else {
                let remainder = user_payout - market.premiums;
                market.premiums = 0;
            ...
            //If no profit, release entire reserve for this option
            market.committed_reserve = market.committed_reserve
            .checked_sub(option.max_potential_payout_in_tokens)
            .ok_or(CustomError::Overflow)?;
        }
        ...
        msg!("Payout usd (in 10^6) {} ", profit_usd);
        msg!("Payout token amount {} ", user_payout);
